 ...
 *
 * Implementation of IP Watch List
 */
...
/**
 * \brief Free global IP watch list context
 *
 * \retval 1 success
 */
int CreateIpWatchListCtxFree()
{
    SCMutexLock(&g_ipwatchlist_ctx->watch_list_ipv4_lock);
    SCRadixReleaseRadixTree(g_ipwatchlist_ctx->watch_list_ipv4_tree);
    SCMutexUnlock(&g_ipwatchlist_ctx->watch_list_ipv4_lock);
    SCMutexLock(&g_ipwatchlist_ctx->watch_list_ipv6_lock);
    SCRadixReleaseRadixTree(g_ipwatchlist_ctx->watch_list_ipv6_tree);
    SCMutexUnlock(&g_ipwatchlist_ctx->watch_list_ipv6_lock);
...
/**
 *  \brief Adds an array of IPs to the IP watch list
 *
 *  \param msg The indicator title copy from the Taxii message
 *  \param adr The array of IPs to be added to the watch list
 *  \param len The number of IPs to be added to the watch list
 *
 *  \retval 1 error
 *  \retval 0 success
 */
...
/**
 * \brief Free function supplied to SCRadixCreateRadixTree to free user supplied data
 *
 * \param user IP Watch List user data
 */
...
/**
 *  \brief Completes initialization of the IP watch list and sets the message
 *
 *  \param msg_header The indicator title copy from the Taxii message
 *  \param data IP Watch list data struct
 *
 *  \retval message
 */
char* InitWatchDataFully(char* msg_header, WatchListData* data)
{
    if (!data->inited) {
