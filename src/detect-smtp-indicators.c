...
int DetectSMTPMatch(ThreadVars *, DetectEngineThreadCtx *, Packet *, Signature *, SigMatch *);
...
void DetectSMPTIndicatorsRegister(void)
{
    ...
    sigmatch_table[DETECT_STIX_SMTP_INDICATORS].Match = DetectSMTPMatch;
 ...
 */
int DetectSMTPMatch(ThreadVars * tv, DetectEngineThreadCtx * de_ctx,
...
#ifdef HAVE_NSS
            static char hash_string[(MD5_LENGTH*2)+1];
            static unsigned char hash_result[MD5_LENGTH+1];
